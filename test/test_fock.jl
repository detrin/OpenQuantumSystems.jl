...
@testset "fock" begin
    ...
    @test destroy(basis) ==
          SparseOperator(basis, sparse(ComplexF64[0 1 0; 0 0 sqrt(2); 0 0 0]))
    @test create(basis) ==
          SparseOperator(basis, sparse(ComplexF64[0 0 0; 1 0 0; 0 sqrt(2) 0]))
    ...
    @test dagger(fockstate(basis, 1)) * destroy(basis) ==
          sqrt(2) * dagger(fockstate(basis, 2))
    ...
    @test dagger(fockstate(basis, 2)) * create(basis) ==
          sqrt(2) * dagger(fockstate(basis, 1))
    ...
    # Test Fock states
    b = FockBasis(5)
    @test expect(number(b), fockstate(b, 3)) == complex(3.0)
