...
@testset "operators-sparse" begin
    ...
    @test 1e-11 >
          D((xbra1 + 0.3 * xbra2) * (op1 + op2), (xbra1 + 0.3 * xbra2) * (op1_ + op2_))
    ...
    @test 1e-12 > D(
        ...
        0.3 * op1_ * dagger(op3_) + 0.3 * op2_ * dagger(op3_),
    )
    ...
    @test I ==
          identityoperator(SparseOpType, b1b) ⊗ identityoperator(SparseOpType, b2b) ⊗
          identityoperator(SparseOpType, b3b)
    ...
    @test I ==
          identityoperator(SparseOpType, b1a) ⊗ identityoperator(SparseOpType, b2a) ⊗
          identityoperator(SparseOpType, b3a)
    ...
    @test 1e-13 >
          D((op1a ⊗ op2a) * dagger(op1b ⊗ op2b), (op1a_ ⊗ op2a_) * dagger(op1b_ ⊗ op2b_))
    @test 1e-13 > D(
        (op1a * dagger(op1b)) ⊗ (op2a * dagger(op2b)),
        (op1a_ * dagger(op1b_)) ⊗ (op2a_ * dagger(op2b_)),
    )
    @test 1e-13 > D(
        (op1a * dagger(op1b)) ⊗ (op2a * dagger(op2b)),
        (op1a_ * dagger(op1b)) ⊗ (op2a_ * dagger(op2b_)),
    )
    ...
    @test diagonaloperator(b, [1.0, 1.0, 1.0, 1.0]) == I
    ...
    OpenQuantumSystems.mul!(result, op, state, complex(1.0), complex(0.0))
    ...
    result = deepcopy(result_)
    OpenQuantumSystems.mul!(result, state, op, complex(1.0), complex(0.0))
    ...
    alpha = complex(1.5)
    beta = complex(2.1)
    OpenQuantumSystems.mul!(result, op', state, alpha, beta) # gemm! with lazy adjoint sparse
    @test 1e-12 > D(result, alpha * op_' * state + beta * result_)
    ...
    alpha = complex(1.5)
    beta = complex(2.1)
    OpenQuantumSystems.mul!(result, state, op', alpha, beta) # gemm! with lazy adjoint sparse
    @test 1e-12 > D(result, alpha * state * op_' + beta * result_)
