...
@testset "master ansatz" begin
    ...
    T = 300
    mu_array = [[2, 1]]
    W01 = thermal_state(T, mu_array, Ham, aggInds; diagonalize=false, diagonal=true)
    rho_traced1 = trace_bath(W01.data, agg, FCProd, aggInds, vibindices; groundState=false)
    mu_array = [[1, 2]]
    W02 = thermal_state(T, mu_array, Ham, aggInds; diagonalize=false, diagonal=true)
    rho_traced2 = trace_bath(W02.data, agg, FCProd, aggInds, vibindices; groundState=false)

    W0 = 1.0*W01 + 0.0*W02 
    W0_bath = get_rho_bath(W0, agg, FCProd, aggInds, vibindices; groundState=false)
