...
@testset "nlevel" begin
    ...
    @test norm(nlevelstate(b, 1)) == 1.0
    @test norm(dagger(nlevelstate(b, 1)) * nlevelstate(b, 2)) == 0.0
    @test norm(dagger(nlevelstate(b, 1)) * transition(b, 1, 2) * nlevelstate(b, 2)) == 1.0
