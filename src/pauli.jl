...
struct PauliBasis{S,B<:Tuple{Vararg{Basis}}} <: Basis
    ...
    function PauliBasis(num_qubits::T) where {T<:Int}
        shape = [2 for _ = 1:num_qubits]
        bases = Tuple(SpinBasis(1 // 2) for _ = 1:num_qubits)
...
"""
abstract type PauliTransferMatrix{
    B1<:Tuple{PauliBasis,PauliBasis},
...
"""
mutable struct DensePauliTransferMatrix{
    B1<:Tuple{PauliBasis,PauliBasis},
    B2<:Tuple{PauliBasis,PauliBasis},
    T<:Matrix{Float64},
} <: PauliTransferMatrix{B1,B2}
    basis_l::B1
    basis_r::B2
    data::T
    function DensePauliTransferMatrix(
        basis_l::BL,
        basis_r::BR,
        data::T,
    ) where {
        BL<:Tuple{PauliBasis,PauliBasis},
...
PauliTransferMatrix(
    ptm::DensePauliTransferMatrix{B,B,Matrix{Float64}},
) where {B<:Tuple{PauliBasis,PauliBasis}} = ptm

function *(
    ptm0::DensePauliTransferMatrix{B,B,Matrix{Float64}},
    ptm1::DensePauliTransferMatrix{B,B,Matrix{Float64}},
) where {B<:Tuple{PauliBasis,PauliBasis}}
...
ChiMatrix(
    chi_matrix::DenseChiMatrix{B,B,Matrix{ComplexF64}},
) where {B<:Tuple{PauliBasis,PauliBasis}} = chi_matrix
...
cache_multiply_pauli_matrices() = begin
    ...
    function _multiply_pauli_matirices(i4::String, j4::String)
        if (i4, j4) ∉ keys(pauli_multiplication_cache)
            pauli_multiplication_cache[(i4, j4)] = mapreduce(
                x -> pauli_multiplication_dict[prod(x)],
                (x, y) -> (x[1] * y[1], x[2] * y[2]),
                zip(i4, j4),
            )
        end
...
function *(
    chi_matrix0::DenseChiMatrix{B,B,Matrix{ComplexF64}},
    chi_matrix1::DenseChiMatrix{B,B,Matrix{ComplexF64}},
) where {B<:Tuple{PauliBasis,PauliBasis}}
    ...
    for ijkl in
        Iterators.product(0:(sop_dim-1), 0:(sop_dim-1), 0:(sop_dim-1), 0:(sop_dim-1))
        i, j, k, l = ijkl
        if (chi_matrix0.data[i+1, j+1] != 0.0) & (chi_matrix1.data[k+1, l+1] != 0.0)
            ...
            ret[
                parse(Int, pauli_product_ik[1], base = 4)+1,
                parse(Int, pauli_product_lj[1], base = 4)+1,
            ] += (
                pauli_product_ik[2] *
                pauli_product_lj[2] *
                chi_matrix0.data[i+1, j+1] *
                chi_matrix1.data[k+1, l+1]
            )
        end
    end
...
function pauli_operators(num_qubits::Int)
    ...
    po = []
    for paulis in Iterators.product((pauli_funcs for _ = 1:num_qubits)...)
...
"""
function PauliTransferMatrix(
    sop::DenseSuperOpType{B,B},
) where {B<:Tuple{PauliBasis,PauliBasis}}
...
SuperOperator(unitary::DenseOpType{B,B}) where {B<:PauliBasis} =
    spre(unitary) * spost(unitary')
...
"""
PauliTransferMatrix(unitary::DenseOpType{B,B}) where {B<:PauliBasis} =
    PauliTransferMatrix(SuperOperator(unitary))
...
function ChiMatrix(unitary::DenseOpType{B,B}) where {B<:PauliBasis}
    ...
    return DenseChiMatrix(
        (unitary.basis_l, unitary.basis_l),
        (unitary.basis_r, unitary.basis_r),
        aj * aj' / (2^num_qubits),
    )
end
...
function PauliTransferMatrix(
    ...
    for (idx, jdx) in Iterators.product(1:sop_dim, 1:sop_dim)
        data[idx, jdx] =
            tr(mapreduce(
                x -> po[idx] * po[x[1]] * po[jdx] * po[x[2]] * chi_matrix.data[x[1], x[2]],
                +,
                Iterators.product(1:16, 1:16),
            ).data) / sop_dim |> real
    end
...
==(
    sop1::T,
    sop2::T,
) where {T<:Union{DensePauliTransferMatrix,DenseSuperOpType,DenseChiMatrix}} =
    sop1.data == sop2.data
==(
    sop1::Union{DensePauliTransferMatrix,DenseSuperOpType,DenseChiMatrix},
