...
function sigmax(b::SpinBasis)
    N = length(b)
    diag = ComplexF64[
        complex(sqrt(real((b.spinnumber + 1) * 2 * a - a * (a + 1)))) for a = 1:N-1
    ]
...
function sigmay(b::SpinBasis)
    N = length(b)
    diag = ComplexF64[
        1im * complex(sqrt(real((b.spinnumber + 1) * 2 * a - a * (a + 1)))) for a = 1:N-1
    ]
    data = spdiagm(-1 => diag, 1 => -diag)
    SparseOperator(b, data)
end
...
function sigmap(b::SpinBasis)
    ...
    diag = ComplexF64[
        complex(sqrt(float(S - m * (m + 1)))) for m = b.spinnumber-1:-1:-b.spinnumber
    ]
    data = spdiagm(1 => diag)
    SparseOperator(b, data)
end
