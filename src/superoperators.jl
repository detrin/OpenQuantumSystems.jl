...
"""
mutable struct SuperOperator{B1<:Tuple{Basis,Basis},B2<:Tuple{Basis,Basis},T} <:
               AbstractSuperOperator{B1,B2}
    basis_l::B1
    basis_r::B2
    data::T
    function SuperOperator{BL,BR,T}(
        basis_l::BL,
        basis_r::BR,
        data::T,
    ) where {BL<:Tuple{Basis,Basis},BR<:Tuple{Basis,Basis},T}
...
SuperOperator{BL,BR}(b1::BL, b2::BR, data::T) where {BL,BR,T} =
    SuperOperator{BL,BR,T}(b1, b2, data)
SuperOperator(b1::BL, b2::BR, data::T) where {BL,BR,T} =
    SuperOperator{BL,BR,T}(b1, b2, data)
SuperOperator(b, data) = SuperOperator(b, b, data)

const DenseSuperOpType{BL<:Tuple{Basis,Basis},BR<:Tuple{Basis,Basis}} =
    SuperOperator{BL,BR,<:Matrix}
const SparseSuperOpType{BL<:Tuple{Basis,Basis},BR<:Tuple{Basis,Basis}} =
    SuperOperator{BL,BR,<:SparseMatrixCSC}
...
DenseSuperOperator(basis_l, basis_r, data) =
    SuperOperator(basis_l, basis_r, Matrix{ComplexF64}(data))
...
SparseSuperOperator(basis_l, basis_r, data) =
    SuperOperator(basis_l, basis_r, SparseMatrixCSC{ComplexF64,Int}(data))
...
Base.length(a::SuperOperator) =
    ...
    length(a.basis_r[1]) *
    length(a.basis_r[2])
samebases(a::SuperOperator, b::SuperOperator) =
    samebases(a.basis_l[1], b.basis_l[1]) &&
    samebases(a.basis_l[2], b.basis_l[2]) &&
    samebases(a.basis_r[1], b.basis_r[1]) &&
...
function *(
    a::SuperOperator{B1,B2},
    b::Operator{BL,BR},
) where {BL<:Basis,BR<:Basis,B1<:Tuple{Basis,Basis},B2<:Tuple{BL,BR}}
    data = a.data * reshape(b.data, length(b.data))
    return Operator(
        a.basis_l[1],
        a.basis_l[2],
        reshape(data, length(a.basis_l[1]), length(a.basis_l[2])),
    )
end

function *(
    a::SuperOperator{B1,B2},
    b::SuperOperator{B2,B3},
) where {B1<:Tuple{Basis,Basis},B2<:Tuple{Basis,Basis},B3<:Tuple{Basis,Basis}}
...
"""
spre(op::AbstractOperator) = SuperOperator(
    (op.basis_l, op.basis_l),
    (op.basis_r, op.basis_r),
    tensor(op, identityoperator(op)).data,
)
...
holds. `op` can be a dense or a sparse operator.
"""
spost(op::AbstractOperator) = SuperOperator(
    (op.basis_r, op.basis_r),
    (op.basis_l, op.basis_l),
    kron(permutedims(op.data), identityoperator(op).data),
)


function _check_input(
    H::AbstractOperator{B1,B2},
    J::Vector,
    Jdagger::Vector,
    rates::Union{Vector{<:Number},Matrix{<:Number}},
...
struct SuperOperatorStyle{BL<:Tuple{Basis,Basis},BR<:Tuple{Basis,Basis}} <:
...
Broadcast.BroadcastStyle(
    ::Type{<:SuperOperator{BL,BR}},
) where {BL<:Tuple{Basis,Basis},BR<:Tuple{Basis,Basis}} = SuperOperatorStyle{BL,BR}()
...
@inline function Base.copy(
    ...
    BL<:Tuple{Basis,Basis},
    BR<:Tuple{Basis,Basis},
    Style<:SuperOperatorStyle{BL,BR},
    Axes,
    F,
    Args<:Tuple,
}
...
function Broadcasted_restrict_f(
    f::BasicMathFunc,
    args::Tuple{Vararg{<:SuperOperator}},
    axes,
)
...
@inline Base.copyto!(
    A::SuperOperator{BL,BR},
    B::SuperOperator{BL,BR},
) where {BL<:Tuple{Basis,Basis},BR<:Tuple{Basis,Basis}} = (copyto!(A.data, B.data); A)
@inline function Base.copyto!(
    dest::SuperOperator{B1,B2},
    bc::Broadcast.Broadcasted{Style,Axes,F,Args},
) where {
    B1<:Tuple{Basis,Basis},
    B2<:Tuple{Basis,Basis},
    B3<:Tuple{Basis,Basis},
    B4<:Tuple{Basis,Basis},
    Style<:SuperOperatorStyle{B3,B4},
    Axes,
    F,
    Args,
}
