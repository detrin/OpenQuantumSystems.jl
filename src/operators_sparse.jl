...
const SparseOpAdjType{BL<:Basis,BR<:Basis} =
    Operator{BL,BR,<:Adjoint{<:Number,<:SparseMatrixCSC}}
const SparseOpType{BL<:Basis,BR<:Basis} =
    Union{SparseOpPureType{BL,BR},SparseOpAdjType{BL,BR}}
...
in the `data` field.
"""
SparseOperator(b1::Basis, b2::Basis, data) =
    Operator(b1, b2, SparseMatrixCSC{ComplexF64,Int}(data))
SparseOperator(b1::Basis, b2::Basis, data::SparseMatrixCSC{ComplexF64,Int}) =
    Operator(b1, b2, data)
...
SparseOperator(b1::Basis, b2::Basis) =
    SparseOperator(b1, b2, spzeros(ComplexF64, length(b1), length(b2)))
...
"""
sparse(a::AbstractOperator) =
    throw(ArgumentError("Direct conversion from $(typeof(a)) not implemented. Use sparse(full(op)) instead."))
...
function expect(
    op::SparseOpPureType{B1,B2},
    state::Operator{B2,B2},
...
function permutesystems(
    ...
    SparseOperator(
        permutesystems(rho.basis_l, perm),
        permutesystems(rho.basis_r, perm),
        data,
    )
end

identityoperator(::Type{T}, b1::Basis, b2::Basis) where {T<:DataOperator} =
    SparseOperator(b1, b2, sparse(ComplexF64(1) * I, length(b1), length(b2)))
...
mul!(
    result::DenseOpType{B1,B3},
    M::SparseOpType{B1,B2},
    b::DenseOpType{B2,B3},
    alpha,
    beta,
) where {B1<:Basis,B2<:Basis,B3<:Basis} =
    (gemm!(alpha, M.data, b.data, beta, result.data); result)
mul!(
    result::DenseOpType{B1,B3},
    a::DenseOpType{B1,B2},
    M::SparseOpType{B2,B3},
    alpha,
    beta,
) where {B1<:Basis,B2<:Basis,B3<:Basis} =
    (gemm!(alpha, a.data, M.data, beta, result.data); result)
mul!(
    result::Ket{B1},
    M::SparseOpPureType{B1,B2},
    b::Ket{B2},
    alpha,
    beta,
) where {B1<:Basis,B2<:Basis} = (gemv!(alpha, M.data, b.data, beta, result.data); result)
mul!(
    result::Bra{B2},
    b::Bra{B1},
    M::SparseOpPureType{B1,B2},
    alpha,
    beta,
) where {B1<:Basis,B2<:Basis} = (gemv!(alpha, b.data, M.data, beta, result.data); result)
