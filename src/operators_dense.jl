...
mutable struct Operator{BL<:Basis,BR<:Basis,T} <: DataOperator{BL,BR}
    ...
    basis_r::BR
    data::T
    function Operator{BL,BR,T}(
        basis_l::BL,
        basis_r::BR,
        data::T,
    ) where {BL<:Basis,BR<:Basis,T}
        (length.((basis_l, basis_r)) == size(data)) ||
            throw(DimensionMismatch("Tried to assign data of size $(size(data)) to bases of length $(length(basis_l)) and $(length(basis_r))!"))
        new(basis_l, basis_r, data)
    end
end
Operator{BL,BR}(basis_l::BL, basis_r::BR, data::T) where {BL,BR,T} =
    Operator{BL,BR,T}(basis_l, basis_r, data)
Operator(basis_l::BL, basis_r::BR, data::T) where {BL,BR,T} =
    Operator{BL,BR,T}(basis_l, basis_r, data)
...
Adapt.adapt_structure(to, x::Operator) =
    Operator(x.basis_l, x.basis_r, Adapt.adapt(to, x.data))
...
"""
DenseOperator(basis_l::Basis, basis_r::Basis, data::T) where {T} =
    Operator(basis_l, basis_r, Matrix{ComplexF64}(data))
DenseOperator(basis_l::Basis, basis_r::Basis, data::Matrix{ComplexF64}) =
    Operator(basis_l, basis_r, data)
DenseOperator(b::Basis, data) = DenseOperator(b, b, data)
DenseOperator(b1::Basis, b2::Basis) =
    DenseOperator(b1, b2, zeros(ComplexF64, length(b1), length(b2)))
DenseOperator(b::Basis) = DenseOperator(b, b)
DenseOperator(op::DataOperator) =
    DenseOperator(op.basis_l, op.basis_r, Matrix{ComplexF64}(op.data))
...
==(x::DataOperator{BL,BR}, y::DataOperator{BL,BR}) where {BL<:Basis,BR<:Basis} =
    (samebases(x, y) && x.data == y.data)
==(x::DataOperator, y::DataOperator) = false
Base.isapprox(
    x::DataOperator{BL,BR},
    y::DataOperator{BL,BR};
    kwargs...,
) where {BL<:Basis,BR<:Basis} = (samebases(x, y) && isapprox(x.data, y.data; kwargs...))
Base.isapprox(x::DataOperator, y::DataOperator; kwargs...) = false

# Arithmetic operations
+(a::Operator{BL,BR}, b::Operator{BL,BR}) where {BL<:Basis,BR<:Basis} =
    Operator(a.basis_l, a.basis_r, a.data + b.data)
...
*(a::Operator{BL,BR}, b::Ket{BR}) where {BL<:Basis,BR<:Basis} =
    Ket{BL}(a.basis_l, a.data * b.data)
*(a::DataOperator, b::Ket) = throw(IncompatibleBases())
*(a::Bra{BL}, b::Operator{BL,BR}) where {BL<:Basis,BR<:Basis} =
    Bra{BR}(b.basis_r, transpose(b.data) * a.data)
*(a::Bra, b::DataOperator) = throw(IncompatibleBases())
*(a::Operator{B1,B2}, b::Operator{B2,B3}) where {B1<:Basis,B2<:Basis,B3<:Basis} =
    Operator(a.basis_l, b.basis_r, a.data * b.data)
...
function *(
    op1::AbstractOperator{B1,B2},
    op2::Operator{B2,B3,T},
) where {B1<:Basis,B2<:Basis,B3<:Basis,T}
    result = Operator{B1,B3,T}(
        op1.basis_l,
        op2.basis_r,
...
function *(
    op1::Operator{B1,B2,T},
    op2::AbstractOperator{B2,B3},
) where {B1<:Basis,B2<:Basis,B3<:Basis,T}
    result = Operator{B1,B3,T}(
        op1.basis_l,
        op2.basis_r,
...
tensor(a::Operator, b::Operator) = Operator(
    tensor(a.basis_l, b.basis_l),
    tensor(a.basis_r, b.basis_r),
    kron(b.data, a.data),
)
...
"""
tensor(a::Ket, b::Bra) = Operator(
    a.basis,
    b.basis,
    reshape(kron(b.data, a.data), length(a.basis), length(b.basis)),
)
...
function expect(
    op::DataOperator{B1,B2},
    state::DataOperator{B2,B2},
) where {B1<:Basis,B2<:Basis}
...
function permutesystems(
    a::Operator{B1,B2},
    perm::Vector{Int},
) where {B1<:CompositeBasis,B2<:CompositeBasis}
...
permutesystems(
    a::AdjointOperator{B1,B2},
    perm::Vector{Int},
) where {B1<:CompositeBasis,B2<:CompositeBasis} = dagger(permutesystems(dagger(a), perm))

identityoperator(::Type{T}, b1::Basis, b2::Basis) where {BL,BR,dType,T<:DenseOpType} =
    Operator(b1, b2, Matrix{ComplexF64}(I, length(b1), length(b2)))
...
@generated function _ptrace(
    ...
    return quote
        ...
        @nloops $RANK ir (d -> 1:shape_r[d]) (d -> (Ir_{d - 1} = Ir_d; Jr_{d - 1} = Jr_d)) (
            d -> (Ir_d += a_strides_r[d]; if !(d in indices)
                Jr_d += result_strides_r[d]
            end)
        ) begin
            @nexprs 1 (d -> (Jl_{$RANK} = 1; Il_{$RANK} = 1))
            @nloops $RANK il (k -> 1:shape_l[k]) (
                k -> (
                    Il_{k - 1} = Il_k; Jl_{k - 1} = Jl_k; if (k in indices && il_k != ir_k)
                        Il_k += a_strides_l[k]
                        continue
                    end
                )
            ) (k -> (Il_k += a_strides_l[k]; if !(k in indices)
                Jl_k += result_strides_l[k]
            end)) begin
...
@generated function _ptrace_ket(
    ::Type{Val{RANK}},
    a::Vector,
    shape::Vector{Int},
    indices::Vector{Int},
    ...
    return quote
        ...
        @nexprs 1 (d -> (Jr_{$RANK} = 1; Ir_{$RANK} = 1))
        @nloops $RANK ir (d -> 1:shape[d]) (d -> (Ir_{d - 1} = Ir_d; Jr_{d - 1} = Jr_d)) (
            d -> (Ir_d += a_strides[d]; if !(d in indices)
                Jr_d += result_strides[d]
            end)
        ) begin
            @nexprs 1 (d -> (Jl_{$RANK} = 1; Il_{$RANK} = 1))
            @nloops $RANK il (k -> 1:shape[k]) (
                k -> (
                    Il_{k - 1} = Il_k; Jl_{k - 1} = Jl_k; if (k in indices && il_k != ir_k)
                        Il_k += a_strides[k]
                        continue
            ...
            ) (k -> (Il_k += a_strides[k]; if !(k in indices)
                Jl_k += result_strides[k]
            end)) begin
...
@generated function _ptrace_bra(
    ::Type{Val{RANK}},
    a::Vector,
    shape::Vector{Int},
    indices::Vector{Int},
...
"""
mul!(
    result::Operator{B1,B3},
    a::Operator{B1,B2},
    b::Operator{B2,B3},
    alpha,
    beta,
) where {B1<:Basis,B2<:Basis,B3<:Basis} =
    (LinearAlgebra.mul!(result.data, a.data, b.data, alpha, beta); result)
mul!(
    result::Ket{B1},
    a::Operator{B1,B2},
    b::Ket{B2},
...
Broadcast.BroadcastStyle(::Type{<:Operator{BL,BR}}) where {BL<:Basis,BR<:Basis} =
    OperatorStyle{BL,BR}()
Broadcast.BroadcastStyle(
    ::OperatorStyle{B1,B2},
    ::OperatorStyle{B3,B4},
) where {B1<:Basis,B2<:Basis,B3<:Basis,B4<:Basis} = throw(IncompatibleBases())

# Out-of-place broadcasting
@inline function Base.copy(
    bc::Broadcast.Broadcasted{Style,Axes,F,Args},
) where {BL<:Basis,BR<:Basis,Style<:OperatorStyle{BL,BR},Axes,F,Args<:Tuple}
...
@inline function Base.copyto!(
    dest::DataOperator{BL,BR},
    bc::Broadcast.Broadcasted{Style,Axes,F,Args},
) where {BL<:Basis,BR<:Basis,Style<:DataOperatorStyle{BL,BR},Axes,F,Args}
...
@inline Base.copyto!(
    A::DataOperator{BL,BR},
    B::DataOperator{BL,BR},
) where {BL<:Basis,BR<:Basis} = (copyto!(A.data, B.data); A)
@inline Base.copyto!(
    dest::DataOperator{BL,BR},
    bc::Broadcast.Broadcasted{Style,Axes,F,Args},
) where {BL<:Basis,BR<:Basis,Style<:DataOperatorStyle,Axes,F,Args} =
