...
arithmetic_unary_error(funcname, x::AbstractOperator) =
    throw(ArgumentError("$funcname is not defined for this type of operator: $(typeof(x)).\nTry to convert to another operator type first with e.g. dense() or sparse()."))
arithmetic_binary_error(funcname, a::AbstractOperator, b::AbstractOperator) =
    throw(ArgumentError("$funcname is not defined for this combination of types of operators: $(typeof(a)), $(typeof(b)).\nTry to convert to a common operator type first with e.g. dense() or sparse()."))
addnumbererror() =
    throw(ArgumentError("Can't add or subtract a number and an operator. You probably want 'op + identityoperator(op)*x'."))
...
*(a::AbstractOperator, b::AbstractOperator) =
    arithmetic_binary_error("Multiplication", a, b)
...
"""
tensor(a::AbstractOperator, b::AbstractOperator) =
    arithmetic_binary_error("Tensor product", a, b)
...
function embed(
    ...
    embed_op = tensor([
        i ∈ indices_sb ? ops_sb[indexin(i, indices_sb)[1]] :
        identityoperator(T, basis_l.bases[i], basis_r.bases[i]) for i = 1:N
    ]...)
...
function embed(
    ...
    all_operators = AbstractOperator[
        identityoperator(T, basis_l.bases[i], basis_r.bases[i]) for i in index_order
    ]
    ...
    perm_order_l = [indexin(idx, expand_order)[1] for idx = 1:length(dims_l)]
    perm_order_r = [indexin(idx, expand_order)[1] for idx = 1:length(dims_r)]

    # Perform the index expansion, the permutation, and the index collapse.
    M = (
        reshape(permuted_op.data, tuple([expand_dims_l; expand_dims_r]...)) |>
        x ->
            permutedims(x, [perm_order_l; perm_order_r .+ length(dims_l)]) |>
...
embed(basis_l::CompositeBasis, basis_r::CompositeBasis, index::Int, op::AbstractOperator) =
    embed(basis_l, basis_r, Int[index], [op])
embed(basis::CompositeBasis, index::Int, op::AbstractOperator) =
    embed(basis, basis, Int[index], [op])
embed(
    basis::CompositeBasis,
    indices::Vector,
    operators::Vector{T},
) where {T<:AbstractOperator} = embed(basis, basis, indices, operators)
embed(basis::CompositeBasis, indices::Vector{Int}, op::AbstractOperator) =
    embed(basis, basis, indices, op)
    ...
    if all([minimum(I):maximum(I);] == I for I in indices)
        for i = 1:N
            if i in complement_indices_flat
                push!(
                    operators_flat,
                    identityoperator(T, basis_l.bases[i], basis_r.bases[i]),
                )
        ...
        complement_operators = [
            identityoperator(T, basis_l.bases[i], basis_r.bases[i])
            for i in complement_indices_flat
        ]
...
embed(
    basis_l::CompositeBasis,
    basis_r::CompositeBasis,
    operators::Dict{Int,T};
    kwargs...,
) where {T<:AbstractOperator} =
    embed(basis_l, basis_r, Dict([i] => op_i for (i, op_i) in operators); kwargs...)
embed(
    basis::CompositeBasis,
    operators::Dict{Int,T};
    kwargs...,
) where {T<:AbstractOperator} = embed(basis, basis, operators; kwargs...)
embed(
    basis::CompositeBasis,
    operators::Dict{Vector{Int},T};
    kwargs...,
...
normalize!(op::AbstractOperator) =
    throw(ArgumentError("normalize! is not defined for this type of operator: $(typeof(op)).\n You may have to fall back to the non-inplace version 'normalize()'."))
...
"""
expect(op::AbstractOperator{B,B}, state::Ket{B}) where {B<:Basis} =
    state.data' * (op * state).data
expect(
    op::AbstractOperator{B1,B2},
    state::AbstractOperator{B2,B2},
) where {B1<:Basis,B2<:Basis} = tr(op * state)
...
function expect(
    indices::Vector{Int},
    op::AbstractOperator{B1,B2},
    state::AbstractOperator{B3,B3},
) where {B1<:Basis,B2<:Basis,B3<:CompositeBasis}
...
function expect(
    indices::Vector{Int},
    op::AbstractOperator{B,B},
    state::Ket{B2},
) where {B<:Basis,B2<:CompositeBasis}
    N = length(state.basis.shape)
    indices_ = complement(N, indices)
    expect(op, ptrace(state, indices_))
end
expect(index::Int, op::AbstractOperator, state) = expect([index], op, state)
expect(op::AbstractOperator, states::Vector) = [expect(op, state) for state in states]
expect(indices::Vector{Int}, op::AbstractOperator, states::Vector) =
    [expect(indices, op, state) for state in states]
...
function variance(
    indices::Vector{Int},
    op::AbstractOperator{B,B},
    state::AbstractOperator{BC,BC},
) where {B<:Basis,BC<:CompositeBasis}
    N = length(state.basis_l.shape)
    indices_ = complement(N, indices)
...
variance(op::AbstractOperator, states::Vector) = [variance(op, state) for state in states]
variance(indices::Vector{Int}, op::AbstractOperator, states::Vector) =
    [variance(indices, op, state) for state in states]
...
"""
exp(op::AbstractOperator) =
    throw(ArgumentError("exp() is not defined for this type of operator: $(typeof(op)).\nTry to convert to dense operator first with dense()."))

permutesystems(a::AbstractOperator, perm::Vector{Int}) =
    arithmetic_unary_error("Permutations of subsystems", a)
...
"""
identityoperator(::Type{T}, b1::Basis, b2::Basis) where {T<:AbstractOperator} =
    throw(ArgumentError("Identity operator not defined for operator type $T."))
identityoperator(::Type{T}, b::Basis) where {T<:AbstractOperator} =
    identityoperator(T, b, b)
identityoperator(op::T) where {T<:AbstractOperator} =
    identityoperator(T, op.basis_l, op.basis_r)
...
samebases(a::AbstractOperator, b::AbstractOperator) =
    samebases(a.basis_l, b.basis_l)::Bool && samebases(a.basis_r, b.basis_r)::Bool
...
multiplicable(a::AbstractOperator, b::AbstractOperator) =
    multiplicable(a.basis_r, b.basis_l)
