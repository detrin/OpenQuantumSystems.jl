...
Molecule(modes::Vector{Mode{C}}, Nvib::T, E::Array{C, 1}) where {C, T} = Molecule{T, C, C}(modes, Nvib, E)

function getMolStateEnergy(
        mol::Molecule{T, C1, C2}, molElState::U, molVibState::Vector{U}
    ) where {T<:Integer, C1<:ComputableType, C2<:ComputableType, U<:Integer}
    energy = mol.E[molElState]
    for mode_i in 1:length(mol.modes)
...
function getMolStateFC(
        ...
        molElState1::U, molVibState1::Vector{U},
        molElState2::U, molVibState2::Vector{U}
    ) where {T<:Integer, C1<:ComputableType, C2<:ComputableType, U<:Integer}
    fc = 1.::C2
    for mode_i in 1:length(mol.modes)
