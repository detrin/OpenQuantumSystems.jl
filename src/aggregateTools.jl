...
"""
function electronic_indices(aggCore::AggregateCore)::ElIndices
...
"""
function vibrational_indices(aggCore::AggregateCore)::VibIndices

    NvibMols = get_nvib(aggCore)
    molLen = length(aggCore.molecules)
    NvibIndMols = Array{Array{Array{Int64,1},1},1}(undef, 0)
    for mol_i = 1:molLen
        push!(NvibIndMols, vibrational_indices(NvibMols[mol_i]))
    end
    molNvib = map((vibInds) -> length(vibInds), NvibIndMols)
    molInds = vibrational_indices(molNvib)
...
* `aggCore`: Instance of [`Aggregate`](@ref).
"""
function get_indices(aggCore::AggregateCore)::Indices
    vibInds = vibrational_indices(aggCore)
    vibIndsLen = length(vibInds)
    elInds = electronic_indices(aggCore)
...
"""
function get_indices_map(aggCore::AggregateCore, aggIndices::Indices)::IndicesMap
...
"""
    ...
    get_franck_condon_factors(aggCore)

Get Frack-Condon factors of the [`Aggregate`](@ref) in a form of matrix.

# Arguments
* `aggCore`: Instance of [`Aggregate`](@ref).
* `aggIndices`: Aggregate indices generated by [`get_indices`](@ref).
"""
function get_franck_condon_factors(aggCore::AggregateCore, aggIndices::Indices)::FCfactorsT
    aggIndLen = length(aggIndices)
    molLen = length(aggCore.molecules)
        ...
        for J = 1:aggIndLen
            ...
            if elind1 == elind2
                ...
                for mi = 1:molLen
                    ...
                    fc *=
                        get_mol_state_fc(mol, elind1[mi], vibind1[mi], elind2[mi], vibind2[mi])
                end
                FC[I, J] = fc
            end
        end
...
get_franck_condon_factors(aggCore::AggregateCore)::FCfactorsT =
    (aggIndices = get_indices(aggCore);
    get_franck_condon_factors(aggCore, aggIndices))
...
function AggregateTools(aggCore::AggregateCore)::AggregateTools
    elIndices = electronic_indices(aggCore)
    vibIndices = vibrational_indices(aggCore)
    indices = get_indices(aggCore)
    indicesMap = get_indices_map(aggCore, indices)
    FCfactors = get_franck_condon_factors(aggCore, indices)
    FCproduct = get_fc_product(aggCore, indices, indicesMap, FCfactors)
